...
- **"Dated extras" block** (end of `script.js`, before Initial Load): a base64 blob decoded at runtime that, from a certain date onward (local time, permanent once passed), inserts a styled link after `#dateDisplay`. It is deliberately opaque because this repo is public and the recipient may read it — do NOT decode it into a comment, name it descriptively, reference what it links to anywhere in this repo (including commit messages), or "clean it up" into readable config. Decode the blob locally if you need to know what it does.
